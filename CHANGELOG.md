...
- **Clickable dependencies in Chart.yaml**: Ctrl+click on a subchart dependency name (when it's a directory in `charts/`, not .tgz) jumps to the subchart's Chart.yaml.
...
- **Completion**: Type `.` after `.Values` or `"` inside `include "` → suggests value paths from resolved values and template names.
